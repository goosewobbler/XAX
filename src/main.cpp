...
// Copyright (c) 2017-2017 The Artax developers
...
#if defined(NDEBUG)
#error "Artax cannot be compiled without assertions."
#endif
...
/** Fees smaller than this (in uxax) are considered zero fee (for relaying and mining)
...
int64_t GetBlockValue(int nHeight)
{
    ...
    if (nHeight == 0) {
        nSubsidy = 5000050 * COIN;
    } else if (nHeight > 0 && nHeight <= 1000) {
        nSubsidy = 0 * COIN;
    } else if (nHeight > 1000 && nHeight <= 435000) {
        nSubsidy = 12 * COIN;
...
int64_t GetMerchantnodePayment(int nHeight, int64_t blockValue, int nMerchantnodeCount)
{
    int64_t ret = 0;

    // No rewards till merchantnode activation.
    ...
    if (pindexPrev != NULL) {
        ...
        if (nHeight != 0 && !IsInitialBlockDownload()) {
            if (!IsBlockPayeeValid(block, nHeight)) {
                mapRejectedBlocks.insert(make_pair(block.GetHash(), GetTime()));
                return state.DoS(0, error("CheckBlock() : Couldn't find merchantnode/budget payment"),
                        REJECT_INVALID, "bad-cb-payee");
            }
        } else {
            if (fDebug)
                LogPrintf("CheckBlock(): Merchantnode payment check skipped on sync - skipping IsBlockPayeeValid()\n");
        }
    }
...
string GetWarnings(string strFor)
{
    ...
    if (!CLIENT_VERSION_IS_RELEASE)
        strStatusBar = _("Ensure you update to version 2.0.0.1 before block 435,000.");
...
bool static AlreadyHave(const CInv& inv)
{
    ...
    case MSG_BUDGET_VOTE:
        if (budget.mapSeenMerchantnodeBudgetVotes.count(inv.hash)) {
            merchantnodeSync.AddedBudgetItem(inv.hash);
            return true;
        }
        return false;
    case MSG_BUDGET_PROPOSAL:
        if (budget.mapSeenMerchantnodeBudgetProposals.count(inv.hash)) {
            merchantnodeSync.AddedBudgetItem(inv.hash);
            return true;
        }
        return false;
            ...
            if (inv.type == MSG_BLOCK || inv.type == MSG_FILTERED_BLOCK) {
                ...
                if (!pushed && inv.type == MSG_MASTERNODE_WINNER) {
                    if (merchantnodePayments.mapMerchantnodePayeeVotes.count(inv.hash)) {
                        CDataStream ss(SER_NETWORK, PROTOCOL_VERSION);
                        ss.reserve(1000);
                        ss << merchantnodePayments.mapMerchantnodePayeeVotes[inv.hash];
                ...
                if (!pushed && inv.type == MSG_BUDGET_VOTE) {
                    if (budget.mapSeenMerchantnodeBudgetVotes.count(inv.hash)) {
                        CDataStream ss(SER_NETWORK, PROTOCOL_VERSION);
                        ss.reserve(1000);
                        ss << budget.mapSeenMerchantnodeBudgetVotes[inv.hash];
                ...
                if (!pushed && inv.type == MSG_BUDGET_PROPOSAL) {
                    if (budget.mapSeenMerchantnodeBudgetProposals.count(inv.hash)) {
                        CDataStream ss(SER_NETWORK, PROTOCOL_VERSION);
                        ss.reserve(1000);
                        ss << budget.mapSeenMerchantnodeBudgetProposals[inv.hash];
                ...
                if (!pushed && inv.type == MSG_MASTERNODE_ANNOUNCE) {
                    if (mnodeman.mapSeenMerchantnodeBroadcast.count(inv.hash)) {
                        CDataStream ss(SER_NETWORK, PROTOCOL_VERSION);
                        ss.reserve(1000);
                        ss << mnodeman.mapSeenMerchantnodeBroadcast[inv.hash];
                ...
                if (!pushed && inv.type == MSG_MASTERNODE_PING) {
                    if (mnodeman.mapSeenMerchantnodePing.count(inv.hash)) {
                        CDataStream ss(SER_NETWORK, PROTOCOL_VERSION);
                        ss.reserve(1000);
                        ss << mnodeman.mapSeenMerchantnodePing[inv.hash];
...
bool static ProcessMessage(CNode* pfrom, string strCommand, CDataStream& vRecv, int64_t nTimeReceived)
{
    ...
    if (strCommand == "version") {
        ...
        // XAX: We use certain sporks during IBD, so check to see if they are
        ...
        //merchantnode signed transaction
    ...
    // getaddr message mitigates the attack.
        ...
        merchantnodePayments.ProcessMessageMerchantnodePayments(pfrom, strCommand, vRecv);
